...
import { SankaDefaultError } from "../models/errors/sanka-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SankaDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SankaDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SankaDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SankaDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
