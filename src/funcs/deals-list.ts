...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(
          operations
            .ApiRoutersV1CasesPublicApiListPublicCasesRequest$outboundSchema,
        ),
        value,
      ),
  ...
  const path = pathToFunc("/v1/public/deals")();
  ...
  const context = {
    ...
    operationID: "api_routers_v1_cases_public_api_list_public_cases",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, z.array(models.CaseSchema$inboundSchema)),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
