...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install sanka`
...
await client.public.expenses.uploadAttachment({ file: fs.createReadStream('/path/to/file') });

// Or if you have the web `File` API you can pass a `File` instance:
await client.public.expenses.uploadAttachment({ file: new File(['my bytes'], 'file') });

// You can also pass a `fetch` `Response`:
await client.public.expenses.uploadAttachment({ file: await fetch('https://somesite/file') });

// Finally, if none of the above are convenient, you can use our `toFile` helper:
await client.public.expenses.uploadAttachment({
...
1. Via the `SANKA_LOG` environment variable
...
const client = new Sanka({
  logger: logger.child({ name: 'Sanka' }),
